...
template <class T> auto convert(const T &t) { return t; }

template <> auto convert<bool>(const bool &value) {
  auto o = value ? Py_True : Py_False;
  Py_INCREF(o);
  return o;
}

template <> auto convert<std::string>(const std::string &value) {
  return value.c_str();
}

template <class... Args> auto build_value_imp(Args... args) {
  auto format = stringer(BuildValueTypes.at(typeid(Args))...);
  return Py_BuildValue(format.c_str(), args...);
}

template <class... Args> auto build_value(Args... args) {
  return build_value_imp(convert(args)...);
}

template <class... Args> auto arg_parse_tuple(PyObject *obj, Args &... args) {
