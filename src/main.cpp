...
struct StrConst
{
  ...
  constexpr bool operator==(StrConst other) const
  {
    if (str_size != other.str_size)
      return false;
    for (size_t n = 0; n < str_size; n++)
      if (str_ptr[n] != other.str_ptr[n])
        return false;
    return true;
  }

  constexpr std::size_t size() const { return str_size; }
};
...
main()
{
  ...
  std::cout << "Use count:"
            << " " << ptr.use_count() << " " << ptr1.use_count() << " "
            << ptr2.use_count() << " " << ptr3->use_count() << std::endl;
}
